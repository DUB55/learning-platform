...
function FollowCam() {
    const { camera, scene } = typeof window !== 'undefined' ? useFrame((state) => {
        const player = scene.children.find(c => c.type === 'Group' && c.children[0]?.geometry?.type === 'BoxGeometry'); // Hacky find player
        if (player) {
            ...
            <Canvas shadows>
                ...
                <KeyboardControls map={controls}>
                    <GameScene
                        ...
                        onFinish={() => setGameState('won')}
                        onDie={() => setGameState('lost')}
                    />
                </KeyboardControls>
            </Canvas>
