        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    ...
                    const oldData = payload.old; // Note: 'old' might not be fully populated in some setups w/o replica identity full
                    // So we rely on our local state 'previousLevel' if needed, or just check the update.

                    // Actually, payload.old is often empty for key columns only unless REPLICA IDENTITY FULL.
                    // Instead, let's compare with our state

                    if (newData.level > previousLevel) {
                        // LEVEL UP!
                        setNewLevel(newData.level);
                        setPreviousLevel(newData.level); // Update state
                        triggerLevelUp(newData.level);
                    } else if (newData.level > 0 && previousLevel === 0) {
                        // First load catch-up if needed, usually ignore
                        setPreviousLevel(newData.level);
                    } else {
                        // Must update local state tracker regardless to keep in sync for next jump
                        setPreviousLevel(newData.level);
                    }
                }
            )
